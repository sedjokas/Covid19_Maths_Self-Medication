...
ax.plot(t_fine, modeled_cases_fine, 'r-', lw=2.4, label='Model (fitted)')
ax.plot(days[fit_mask], cases_smoothed[fit_mask], 'k.', ms=7, label='Observed (OWID, smoothed)')
ax.plot(days[~fit_mask], cases_smoothed[~fit_mask], 'x', color='gray', ms=8, mew=2, label='Reporting artifact (excluded)')
...
ax.plot(t_fine, modeled_deaths_fine, 'r-', lw=2.4, label='Model (fitted)')
ax.plot(days, deaths_net, 'k.', ms=7, label='Observed (OWID)')
ax.set_xlabel('Day (from 1 June 2021)')
...
fig, ax = plt.subplots(figsize=(5, 4))
ax.plot(t_fine, E, 'orange', lw=2.2, label='E')
ax.plot(t_fine, I, 'g-', lw=2.2, label='I')
...
fig, ax = plt.subplots(figsize=(5, 4))
ax.plot(t_fine, H, 'purple', lw=2.2, label='H (hospitalized)')
ax.plot(t_fine, M, 'brown', lw=2.2, label='M (home care)')
...
fig, ax = plt.subplots(figsize=(5, 4))
ax.plot(t_fine, R, 'teal', lw=2.2, label='R')
ax.set_xlabel('Day'); ax.set_ylabel('R per 1,000', color='teal')
ax2 = ax.twinx()
ax2.plot(t_fine, D_people, 'k--', lw=2.2, label='D (cumulative deaths, real people)')
...
fig, ax = plt.subplots(figsize=(5, 4))
ax.plot(gammas, peakM_vs_gamma, 'o-', color='brown', lw=2.2, ms=6)
ax.axvline(FIXED['gamma'], color='gray', ls='--', lw=1.5)
...
fig, ax = plt.subplots(figsize=(5, 4))
ax.plot(gammas, deaths_vs_gamma, 'o-', color='indianred', lw=2.2, ms=6)
ax.axvline(FIXED['gamma'], color='gray', ls='--', lw=1.5)
ax.set_xlabel(r'$\gamma$ (hospitalization proportion)')
ax.set_ylabel('Cumulative deaths')
